...
#################        FUNCTION TO CREATE EMPLOYEE          ################# 
    def create_employee(self, emp_id, last_name, middle_name, first_name, contact_no, email_id,  password,  ssn, dob, tier, active=1,  address=""):
        ...
        query = """
                ...
                VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d);""" % (emp_id, last_name, middle_name, 
...
# #################        FUNCTION TO UPDATE EMPLOYEE ACCOUNT INFO                     ################# 
    def update_account_info(self, emp_id, last_name, middle_name, first_name, contact_no, email_id, ssn,  dob, address, tier):
        
        query = """
            UPDATE Employees  
            SET emp_id='%s', last_name='%s', middle_name='%s', first_name='%s', dob='%s', contact_no='%s', email_id='%s', address='%s', ssn='%s',  tier=%d 
            WHERE emp_id='%s';""" % (emp_id, last_name, middle_name, first_name, dob, contact_no, email_id, address, ssn,  int(tier), emp_id)
        cursor.execute(query) 
        try:
            db.commit()
            # result = cursor.fetchall()
...
#################        FUNCTION TO ADD TRANSACTIONS (APPROVER: TIER2)                    ################# 
    def add_transaction(self,account1, account2, amount):
        ...
        if float(amount) > 1000:
            approver = 2
        query = """ 
                    INSERT into Transactions(from_account, to_account, approver1_id, approver2, amount, status) 
                    VALUES(%d, %d, '-1', %d, %d, 1)
