...
#################        FUNCTION TRANSFER FUNDS                    ################# 
    def fund_transfers(self, account1, account2, amount, transaction_no=-1):
        ...
        query = """ 
            ...
            """ % (float(amount), int(account1)) 
        cursor.execute(query) 
        
        query = """ 
                UPDATE Accounts SET balance=balance + %f where account_no = %d; 
            """ % (float(amount), int(account2)) 
        cursor.execute(query) 
...
#################        FUNCTION TO DEBIT FUNDS                     ################# 
    def debit_request(self, account, amount):
        query = """ 
                Select balance, active from Accounts where account_no = %d; 
            """ % (account) 
        cursor.execute(query) 
        result = cursor.fetchall()
        # bal = result[0][0]
...
#################                 FUNCTION TO DEPOSIT CHECK                 #################    
    def deposit_check(self, cheque_no):
        query = """
            SELECT to_account, from_account, amount, active FROM Cheque WHERE cheque_no=%d; """ % (int(cheque_no))
        cursor.execute(query) 
        result = cursor.fetchall()
        # print(len(result))
        if len(result) == 0:
...
################                    FUNCTION TO GET CHEQUE LIST                 #################    
    def get_cheque_list(self, userid):
        ...
        INNER JOIN Accounts AS ac ON c.from_account = ac.account_no 
