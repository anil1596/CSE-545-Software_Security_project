...
logging.basicConfig(filename='SystemLogs/bank.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s')
...
@application.route('/registerCustomer', methods=['POST'])
def registerCustomer():
    ...
    if temp == 1:
        ...
        return redirect(url_for('get_customer_dashboard_ui'))
...
def login():
    ...
    if values['usertype'] == 'customer':
        ...
        if  emp.verify_employee(values['userid'], values['password']) == 1:
            ...
            if emp_tier == 3:
                return redirect(url_for('get_admin_dashboard_ui'))
            else: 
                return redirect(url_for('get_tier'+ str(emp_tier) + '_dashboard_ui'))
        else:
            response = {
...
def fund_transfers():
    ...
    # c = Customers()
    # if c.check_user_id(values['userid']) and 
...
def deposit_fund():
    ...
    if values['userid'] in session:     
        c = Customers()
        response = {
            'message' : c.credit_request(values['account'], values['amount'] )
        }
        return jsonify(response), 200
    else:
        print('Not logged In')
...
@application.route('/getTransactionHistory', methods=['POST'])
...
def deposit_cheuqe():
    ...
    if values['userid'] in session:   
        c = Customers()
        response = {
                'message' : c.deposit_check(values['cheque_no'])
            }
        return jsonify(response), 200
    else:
        print('Not logged In')
